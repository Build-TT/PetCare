...
export default async function handler(request, response) {
  if (request.method !== 'POST') return response.status(405).json({ status: 'error', message: 'POST required' })

  const secret = process.env.LINE_CHANNEL_SECRET || ''
  const gasUrl = process.env.VITE_GAS_URL || ''
  const relaySecret = process.env.GAS_WEBHOOK_SECRET || ''
  if (!secret || !gasUrl || !relaySecret) return response.status(500).json({ status: 'error', message: 'Webhook relay is not configured' })
  ...
  try {
    ...
    if (!Array.isArray(payload.events)) return response.status(400).json({ status: 'error', message: 'Invalid LINE webhook payload' })

    const gasResponse = await postToGas(gasUrl, JSON.stringify({ action: 'lineWebhookRelay', relay_secret: relaySecret, payload }))
    const gasText = await gasResponse.text()
    let gasBody = null
    try { gasBody = JSON.parse(gasText) } catch { /* GAS response is still checked by HTTP status. */ }
